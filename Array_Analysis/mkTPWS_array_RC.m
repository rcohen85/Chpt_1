% mkTPWS.m
% Script takes .mat files containing click start/end times for each .wav
% file and combines it in one large file
...
% Setup variables:
baseDir = 'G:\TA_Vis\GU1703\Gg\SPICE_metadata'; % directory containing .mat files 
outDir = 'G:\TA_Vis\GU1703\Gg\TPWS'; % directory where you want to save your TPWS files
name = 'Gg'; % output file name (species & sampling rate; site & deployment, etc)
ppThresh = 118; % minimum RL in dBpp. If detections have RL below this
...
%for itr0 = 1:length(dirSet)
    %if ~strcmp(dirSet(itr0).name,'.')&&~strcmp(dirSet(itr0).name,'..')...
        ...
        lfs = length(fileSet);
        for itr1 = 1:lfs
            thisFile = fileSet(itr1).name;
            
            load(fullfile(baseDir,thisFile),'-mat','pos','rawStart',...
                'ppSignal','specClick','fs','yFilt','f')
            ...
            if ~isempty(pos)
                ...
                rawStartDnum = datenum(rawStart,'yyyymmdd_HHMMSS');
...
%                     datenum([2000,0,0,0,0,0]);
        ...
        outFileName = sprintf('%s_TPWS1.mat',name);
