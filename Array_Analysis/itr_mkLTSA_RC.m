...
clearvars
clear global
filePrefix = 'WAT_BS_01'; % File name to match. 
...
srate = 200; % sample rate (kHz)
LTSApath = 'I:\WAT_BS_01_Detector\WAT_BS_01_LTSA'; % directory containing all LTSAs for this deployment
% LTSA folder should match the site specified in prefix
tpwsPath = 'I:\WAT_BS_01_Detector\WAT_BS_01_TPWS'; %directory of TPWS files
%tfName = 'E:\Data\TFs'; % Directory ...
...
for iD = 19:length(fileMatchIdx)
    matchingFile = fileList{fileMatchIdx(iD)};
    detfn = dir(fullfile(tpwsPath,matchingFile));
    
    if exist('tfName','var')
        mkLTSAsessions_old('filePrefix', filePrefix, 'detfn',detfn.name,...
