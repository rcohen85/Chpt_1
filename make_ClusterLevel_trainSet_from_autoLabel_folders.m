...
test_saveFileName = 'TestSet_MSPICIWV_ntest_noReps.mat';
...
for iU = 1:length(myTypeList)
    ...
    if length(testSelect)>ntest
        expandedTestList = randsample(testSelect,ntest); 
    else
        expandedTestList = randsample(testSelect,ntest,true);% beef up tiny sample size.
    end
    
    trainSelect = setdiff(1:nItems,expandedTestList);
    if length(trainSelect)>ntrain
        expandedTrainList = randsample(trainSelect,ntrain); 
    else
        expandedTrainList = randsample(trainSelect,ntrain,true);% beef up tiny sample size.
    end
